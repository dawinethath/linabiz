...
<script type="text/x-kendo-template" id="vendor-dashboard-tmpl">	
	<div class="navbar">
		<div class="navbar-inner">
			<ul class="nav">
				<li>
					<a href="#/vendors/u/new"><i class="icon-user"></i></a>
				</li>
				<li>
	...
	<div>
		<div class="span3" data-bind="click: getPO" style="color: white; font-size: 20px; background-color:Purple; border:0px solid black; padding: 10px;">
			បញ្ជាទិញ
			<br><br>
		...
		<div class="span3" data-bind="click: overDueBills" style="color: white; font-size: 20px; background-color:Crimson; border:0px solid black; padding: 10px;">
			វិក្កយប័ត្រ
			<br><br>
	...
	<div class="navbar">
		<div class="navbar-inner">
			<ul class="nav">
				...
				<li>
					<a href="#/bills/u/new"><i class="icon-billស"></i>ទិញ</a>
				</li>
				<li>
					<a href="#/bills/u/new"><i class="icon-bill"></i>ចំណាយ</a>
				</li>
			</ul>
		</div>
	</div>
	<div class="row-fluid">
		<div class="span3" data-bind="click: toDashboard" style="color: white; font-size: 20px; background-color:MediumSeaGreen; border:0px solid black; padding: 10px;">
			<span><i class="icon-dashboard large"></i> ទៅទំព័រអ្នកផ្គត់ផ្គង</span>
			<br><br>
			<span></span>
			<span style="font-size: 14px">&nbsp;</span>
		</div>
...
<script type="text/x-kendo-template" id="vendor-detail-tmpl">
	<table class="table">
		<thead>
			<tr>
				<th colspan="4">
...
<script type="text/x-kendo-template" id="vendors-bill-form-header-tmpl">
		...
		<table class="table">
			...
			<tbody>
				<tr>
						...
						<td><input type="checkbox" data-bind="checked: paidByCash" /></td>
						<td width="200">គណនី</td>
						<td><input id="accountDropdownlist" type="text" style="width: 220px;" /></td>
				</tr>
				<tr>
					<td>អ្នកផ្គត់ផ្គង</td>
	...
	<div id="bill-form-list-container" class="container-960"></div>
</script>
...
<script>
	...
	banhji.segment = kendo.observable({
		...
		itemStore: new kendo.data.DataSource(),
		addItems : function() {},
		addNew 	 : function() {},
		find 	 : function() {
	...
	**********************/
				...
				if(res) {
					...
					banhji.journalEntry.save().then(
						function(res){
							...
							$("#payBillBtn").html("ត្រូវការបានសំរេច។");
						},
						function(error) {
							$("#payBillBtn").html("បង់ប្រាក់");
						}
			...
			banhji.bill.dataSource.query({
				...
				$.each(data, function(i, v){
					...
					banhji.bill.get('paymentArr').push({
						...
						contact: {id: v.vendor.id, name: v.vendor.name, surname: v.vendor.surname},
						fine: 0.00, 
						payment_date: new Date(), 
						reference: v, 
						type:'bill', 
	...
	banhji.view = {
		...
		billForm 	: new kendo.Layout("#vendors-bill-form-header-tmpl", {model: banhji.bill}),
		billExp 	: new kendo.View("#vendors-bill-form-expense-list-tmpl", {model: banhji.bill}),
		billPur 	: new kendo.View("#vendors-bill-form-purchase-list-tmpl", {model: banhji.bill}),
	...
	banhji.router.route('/bills/purchase/new', function(){
		if(!auth.getLogin()) {
			...
			banhji.view.layout.showIn('#content', banhji.view.billForm);
			// banhji.view.billForm.showIn("#bill-form-list-container", banhji.view.billPur);
			var account = $("#accountDropdownlist").kendoDropDownList({
				...
				autoBind: false,
				open: function(e){
					if(banhji.bill.get("paidByCash") === true) {
						e.sender.dataSource.filter({field: 'account_type_id', operator: '', value: 6});
					} else {
						e.sender.dataSource.filter({field: 'account_type_id', operator: '', value: 11});
					}
				}
	...
	// account section
	banhji.router.route('/accounts', function(){
		// list all accounts
	});

	banhji.router.route('/accounts/:id', function(id){
		// display accounts and its activity based on given id
	});
	banhji.router.route('/accounts/u/list', function(){
		banhji.view.layout.showIn('#content', banhji.view.vendorList);
			$("#vendorListView").kendoListView({
				dataSource: banhji.account.dataStore,
				template: "<div>#:code#&nbsp;#:name#</div>",
				selectable: true,
				change: function() {
					var selected = this.select();
					var model = this.dataItem(selected);
					banhji.account.setCurrent(model);
				}
			});
	});

	banhji.router.route('/gl', function(){
		// display gl with given date (if not beginning of the year)
	});
	...
	banhji.router.route('/gl/record', function(){
		// display accounts and its activity based on given id
	});
